...
trait Uploader
{
    ...
    protected function validateFile($file)
    {
        $validator = Validator::make([
            ...
            'file' => 'file|mimetypes:image/*|mimes:jpg,jpeg,png|max:' . (file_upload_max_size(config('autocrud.max_filesize.file.image')) / 1024),
        ], [
    ...
    public function uploadMaster($file, $path_directory = null)
    {
        ...
        $thumbs = config('autocrud.image.thumbs');
    ...
    protected function createBaseName($filename)
    {
        ...
        $check = Model::where('basename', $basename)->count();
        if ($check > 0) {
    ...
    protected function saveToStorage($file, $directory = '')
    {
        ...
        if (config('autocrud.image.origin_maximum_width')) {
            if ($image->width() > config('autocrud.image.origin_maximum_width')) {
                //mencegah uploadan dengan ukuran dan kualitas terlalu tinggi
                $image->resize(config('autocrud.image.origin_maximum_width'), null, function (Constraint $constraint) {
        ...
        $image = $image_return->encode($this->current_extension, config('autocrud.image.quality'));
        ...
        if (config('autocrud.image.enable_webp')) {
            $image = $image_return->encode('webp', config('autocrud.image.quality'));
    ...
    protected function generateThumbnail($image, $directory = '', $thumbname = '', $thumbconfig = [])
    {
        ...
        $image_store = $image->encode($this->current_extension, config('autocrud.image.quality'));
        Storage::put($this->createPath($directory, $thumb_filename), (string) $image_store);
        if (config('autocrud.image.enable_webp')) {
            $image_store = $image->encode('webp', config('autocrud.image.quality'));

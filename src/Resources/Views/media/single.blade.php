...
<div class="input-image-holder text-center" data-hash="{{ $hash }}">
	...
	<div class="pull-left float-start {{ $mode == 'vertical' ? 'float-left' : '' }}">
		<a href="#" class="trigger-upload-image">
			<img data-fallback="{{ defaultImage() }}" src="{{ isset($media->id) ? $media->url('thumb') : $media->fallback() }}" alt="Image Uploader" class="avatar rounded-circle media-item">
		</a>
	</div>
	<div class="pull-right float-start {{ $mode == 'vertical' ? 'float-left' : '' }} px-2">
